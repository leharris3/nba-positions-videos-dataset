...
def extract_frames(video_path, frames_dir, bbox, overwrite=False, start=-1, end=-1, every=1):
    """
...
def video_to_frames(video_path, frames_dir, bbox, overwrite=False, every=1, chunk_size=1000):
    """
    ...
    with ThreadPoolExecutor() as executor:
        ...
        futures = [
            executor.submit(
                extract_frames, video_path, frames_dir, bbox, overwrite, f[0], f[1], every
            )
...
if __name__ == "__main__":
    video_to_frames(
        video_path="test.mp4", frames_dir="test_frames", overwrite=False, every=5
    )
