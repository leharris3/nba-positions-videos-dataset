...
from phalp.utils.utils_detectron2 import (
...
class PHALP(nn.Module):
    ...
    def track(self, video_fp:str=None, pkl_out_fp:str=None):
        ...
        self.setup_deepsort()
        log.info("Set up DeepSort...")

        self.default_setup()
        log.info("Default set up completed...")
    ...
    def forward_for_tracking(self, vectors, attibute="A", time=1):
        ...
        if attibute == "P":
            ...
            with torch.no_grad():
                pose_pred = self.pose_predictor.predict_next(
                    en_pose, en_data, en_time, time
                )
        ...
        if attibute == "L":
            ...
            for bs in range(BS):
                ...
                if (
                    self.cfg.phalp.distance_type == "EQ_020"
                    or self.cfg.phalp.distance_type == "EQ_021"
                ):
                    loc_ = 1
                else:
    ...
    def get_pose_distance(self, track_pose, detect_pose):
        ...
        track_pose2, detect_pose2 = np.square(track_pose).sum(axis=1), np.square(
            detect_pose
        ).sum(axis=1)
        r2 = (
    ...
    def cached_download_from_drive(self, additional_urls=None):
        ...
        if not os.path.exists(smpl_path):
            ...
            convert_pkl(
                "/mnt/arc/levlevi/nba-positions-videos-dataset/4d-pose-extraction/PHALP/phalp/data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl"
            )
            os.system(
                "rm /mnt/arc/levlevi/nba-positions-videos-dataset/4d-pose-extraction/PHALP/phalp/data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl"
            )
